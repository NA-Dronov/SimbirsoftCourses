...
namespace HtmlCorrectorApp_Tuple
{
    public class HtmlCorrector
    {
        ...
        /// <summary>
        /// Массив для хранения словаря
        /// </summary>
        private string[] dictionary = new string[1000];
        /// <summary>
        /// Размер словаря
        /// </summary>
        public int DictSize { get { return dictionary.Length; } }
        /// <summary>
            ...
            try
            {
                ...
                using (StreamReader reader = File.OpenText(DictLocation))
                {
                    ...
                    while (((input = reader.ReadLine()) != null) && (numOfLine < 100000))
                    {
                        ...
                        if (numOfLine == dictionary.Length)
                            Array.Resize(ref dictionary, dictionary.Length + 1000);
                        dictionary[numOfLine] = input;
                        numOfLine++;
                    }
        ...
        public void CorrectHtml()
        {
            try
            {
                ...
                using (StreamReader reader = File.OpenText(InputFileName))
                {
                    ...
                    Tuple<string, string> format = new Tuple<string, string>("<b>", "</b>");
                        ...
                        using (StreamWriter writer = File.CreateText(OutputFileName + (i++) + ".html"))
                        {
                            ...
                            while (((input = reader.ReadLine()) != null) && (string_count < N))
                            {
                                ...
                                while (m.Success)
                                {
                                    ...
                                    {
                                        input = input.Insert(m.Index + shift, format.Item1)
                                            .Insert(m.Index + shift + m.Length + 3, format.Item2);
                                        shift += format.Item1.Length + format.Item2.Length; //Количество символов в <b></b>
                                    }
                                    m = m.NextMatch();
                                }
        ...
        public override string ToString()
        {
            ...
            return string.Format("Input  file: {0}\n" + 
                ...
                " 1.File Name - {3}\n" + 
                " 2.Size      - {4}",
                InputFileName, OutputFileName, N, DictLocation, dictionary.Length);
        }
        /// <summary>
